...
def AddMethods(project_wasteandmaterial, db_wasteandmaterial_name):
    ...
    for key, value in dic.items():
        m_unit = value["unit"]
        m_code = value["code"]
        m_name = value["name"].replace("kilogram", "solid").replace("cubicmeter", "liquid")
        m_type = value["type"]
...
def CheckMethods(project_wasteandmaterial):
    ...
    methods_wasteandmaterial = [m for x in list(bd.methods) if "WasteAndMaterial Footprint" == x[0]]
