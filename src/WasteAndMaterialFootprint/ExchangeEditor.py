...
def ExchangeEditor(project_wasteandmaterial, db_name, db_wasteandmaterial_name):
    ...
    for NAME, df in file_dict.items():
        ...
        for exc in tqdm(df.to_dict('records'), desc=f"**  {progress_db} - {NAME}: "):
            ...
            process.new_exchange(input=wasteandmaterial_ex.key, amount=amount, unit=unit, type='biosphere').save()
        ...
        log_entry = (end.strftime("%m/%d/%Y, %H:%M:%S"), db_name, NAME, "additions", count, "duration:", str(duration))
